...
def test_agent_tools_are_function_tools():
    ...
    for tool in tools:
        ...
        assert hasattr(tool, "on_invoke_tool"), (
            f"Tool {tool} should have 'on_invoke_tool' method"
        )
...
def test_task_tools_have_correct_names():
    ...
    for expected_name, tool in expected_names.items():
        assert tool.name == expected_name, (
            f"Tool name should be '{expected_name}', got '{tool.name}'"
        )
...
def test_agent_has_instructions():
    ...
    from ai_agents.agent import AGENT_INSTRUCTIONS, task_manager_agent
...
def test_create_task_schemas():
    ...
    from ai_agents.schemas import TaskInfo, TaskListResult, TaskOperationResult

    # Test TaskOperationResult
    result = TaskOperationResult(
        task_id="test-123", status="created", title="Test Task"
    )
    ...
    # Test TaskListResult
    task_list = TaskListResult(tasks=[task_info], total=1)
