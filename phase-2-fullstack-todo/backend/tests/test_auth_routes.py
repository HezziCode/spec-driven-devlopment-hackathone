...
def test_signup_duplicate_username(
    client, session, create_test_user, valid_signup_data
):
    ...
    assert (
        "username" in detail["error"].lower()
        or "already exists" in detail["error"].lower()
    )
...
def test_signup_password_hashed_in_db(client, session, valid_signup_data):
    ...
    user = session.exec(
        select(User).where(User.email == valid_signup_data["email"].lower())
    ).first()
...
def test_login_wrong_password(client, session, create_test_user):
    ...
    user, _ = create_test_user(email="login@example.com", password="SecurePass123")
...
def test_login_invalid_email_format(client, session):
    ...
    response = client.post(
        "/auth/login", json={"email": "notanemail", "password": "SomePassword123"}
    )
...
def test_login_jwt_token_valid(client, session, create_test_user):
    ...
    user, plain_password = create_test_user(
        email="token@example.com", password="SecurePass123"
    )

    response = client.post(
        "/auth/login", json={"email": "token@example.com", "password": "SecurePass123"}
    )
...
def test_login_password_verification(client, session, create_test_user):
    ...
    user, _ = create_test_user(
        email="verify@example.com", password="CorrectPassword123"
    )

    # Correct password should succeed
    response1 = client.post(
        "/auth/login",
        json={"email": "verify@example.com", "password": "CorrectPassword123"},
    )
    ...
    response2 = client.post(
        "/auth/login",
        json={"email": "verify@example.com", "password": "WrongPassword123"},
    )
...
def test_logout_with_token(client, session, create_test_user):
    ...
    login_response = client.post(
        "/auth/login", json={"email": "logout@example.com", "password": "Pass123"}
    )
